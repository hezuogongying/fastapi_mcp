...
from pydantic import BaseModel  # 导入 Pydantic 用于数据校验和模型定义
...
class Item(BaseModel):
    ...
    id: int  # 物品的唯一标识符
    name: str  # 物品的名称
    description: Optional[str] = None  # 物品的描述，可选字段
    price: float  # 物品的价格
    tags: List[str] = []  # 物品的标签列表，默认为空列表


items_db: dict[int, Item] = {}  # 模拟数据库，用于存储物品数据，键为物品ID，值为物品对象
